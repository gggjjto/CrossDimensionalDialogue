...
class ImageMessageBase(SQLModel):
    ...
    quality_score: Optional[float] = Field(default=None, ge=0, le=1, description="质量评分")
    thumbnail_url: Optional[str] = Field(default=None, max_length=500, description="缩略图URL")
    compressed_url: Optional[str] = Field(default=None, max_length=500, description="压缩图片URL")
    metadata: Optional[dict] = Field(default=None, sa_type=JSON, description="元数据")
...
class FileMessageBase(SQLModel):
    ...
    checksum: Optional[str] = Field(default=None, max_length=64, description="文件校验和")
    metadata: Optional[dict] = Field(default=None, sa_type=JSON, description="元数据")
    preview_url: Optional[str] = Field(default=None, max_length=500, description="预览URL")
    download_count: int = Field(default=0, description="下载次数")
    processing_status: ProcessingStatus = Field(default=ProcessingStatus.PENDING, description="处理状态")
    security_scan_status: SecurityScanStatus = Field(default=SecurityScanStatus.PENDING, description="安全扫描状态")
...
class FileProcessingTaskBase(SQLModel):
    ...
    result_data: Optional[dict] = Field(default=None, sa_type=JSON, description="结果数据")
...
class FileProcessingTaskCreate(FileProcessingTaskBase):
    ...
    file_message_id: Optional[uuid.UUID] = Field(default=None, description="关联的文件消息ID")
    image_message_id: Optional[uuid.UUID] = Field(default=None, description="关联的图片消息ID")
...
class FileProcessingTask(FileProcessingTaskBase, table=True):
    ...
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    file_message_id: Optional[uuid.UUID] = Field(default=None, foreign_key="file_messages.id", description="关联的文件消息ID")
    image_message_id: Optional[uuid.UUID] = Field(default=None, foreign_key="image_messages.id", description="关联的图片消息ID")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="创建时间")
