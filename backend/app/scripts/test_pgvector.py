...
def test_pgvector_extension():
    ...
    with Session(engine) as session:
        try:
            ...
            result = session.exec(text("SELECT extname, extversion FROM pg_extension WHERE extname = 'vector'")).first()
            if result:
            ...
            # 2. 检查 pgvector 版本
            print("📋 检查 pgvector 版本...")
            version_result = session.exec(text("SELECT vector_version()")).first()
            if version_result:
                print(f"✅ pgvector 版本: {version_result[0]}")
            else:
                print("❌ 无法获取 pgvector 版本")
                return False
            ...
            # 测试向量相似度搜索
            print("🔍 测试向量相似度搜索...")
            similarity_result = session.exec(text("""
            ...
            # 测试余弦相似度
            print("🔍 测试余弦相似度...")
            cosine_result = session.exec(text("""
