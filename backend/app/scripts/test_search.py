...
def test_text_search():
    ...
    with Session(engine) as db:
        ...
        search_request = CharacterSearchRequest(
            query="苏格拉底", search_type="text", limit=10
        )
        ...
        print("\n2. 按描述搜索 '哲学':")
        search_request = CharacterSearchRequest(
            query="哲学", search_type="text", limit=10
        )
        result = character.search(db, search_request=search_request)
        print(f"找到 {result.total} 个结果:")
        for item in result.results:
        ...
        print("\n3. 按人格描述搜索 '魔法':")
        search_request = CharacterSearchRequest(
            query="魔法", search_type="text", limit=10
        )
        result = character.search(db, search_request=search_request)
        print(f"找到 {result.total} 个结果:")
        for item in result.results:
        ...
        if philosophy_tag:
            search_request = CharacterSearchRequest(
                query="", search_type="text", limit=10, tag_ids=[philosophy_tag.id]
            )
            result = character.search(db, search_request=search_request)
            print(f"找到 {result.total} 个结果:")
            for item in result.results:
        ...
        # 测试5: 空查询
        print("\n5. 空查询 (获取所有角色):")
        search_request = CharacterSearchRequest(query="", search_type="text", limit=10)
        result = character.search(db, search_request=search_request)
        print(f"找到 {result.total} 个结果:")
        for item in result.results:
            print(f"  - {item.character.name}: {item.character.short_bio}")
...
def test_vector_search():
    ...
    with Session(engine) as db:
        search_request = CharacterSearchRequest(
            query="智慧", search_type="vector", limit=10
        )
        result = character.search(db, search_request=search_request)
...
def test_hybrid_search():
    ...
    with Session(engine) as db:
        search_request = CharacterSearchRequest(
            query="科学", search_type="hybrid", limit=10
        )
        result = character.search(db, search_request=search_request)
